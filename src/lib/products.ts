    ...
    return data
      ...
      .map((p: any) => {
        ...
        return {
          ...
          sizes: Array.from(new Set((p.variants ?? []).map((v: any) => String(v.size)).filter(Boolean))),
          colors: Array.from(new Set((p.variants ?? []).map((v: any) => v.color).filter(Boolean))).map((c: string) => ({ label: c, hex: "#888" })),
